...
def load_model(model_name: str, model_info: Dict):
    ...
    if model_name in {
        ...
        model = joint_runtime.load_model(
            join(DATA_ROOT, model_info["model_path"]), join(DATA_ROOT, model_info["model_opts_path"])
        )
    elif model_name == "Stemmer":
        model = stemmer_runtime.load_model(join(DATA_ROOT, model_info["model_path"]))
    elif model_name == "NER":
        model = ner_runtime.load_model(
            join(DATA_ROOT, model_info["model_path"]), join(DATA_ROOT, model_info["model_opts_path"])
        )
    elif model_name == "SemanticParser":
        model = semantic_runtime.load_model(join(DATA_ROOT, model_info["model_path"]))
    else:
...
def process_text(model_name: str, model_info: Dict, text: str):
    ...
    if model_name == "JointModel-All":
        ...
        doc.morph_tag = joint_runtime.predict_morpheme_tags_file(model, text)["doc"]
        doc.morph_tag_conll = joint_runtime.predict_morpheme_tags_file(model, text)[
            "conllu"
        ]
